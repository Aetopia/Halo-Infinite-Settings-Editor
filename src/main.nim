import wNim/[wApp, wFrame, wPanel, wListCtrl, wStaticLine, wButton,
        wMessageDialog, wTextEntryDialog]
import winim/shell
import algorithm
import json
import os
...
proc getGamePlatCfgFile: string =
    ...
    MessageDialog(nil, "\"SpecControlSettings.json\" doesn't exist!",
            "Halo Infinite Settings Editor", wOk or wIconExclamation).display()
    quit(1)
...
proc searchSettings(list: wListCtrl, query: string): seq[int] =
    ...
    for i in keys:
        for j in [0, 1]:
            for k in [wListStateFocused, wListStateSelected,
                    wListStateDropHighlighted, wListStateCut, 0x00000003]:
...
if isMainModule:
    ...
    let
        ...
        list = panel.ListCtrl(style = wLcReport or wLcNoHeader or wLcSingleSel,
                size = (795, 547), pos = (0, 23))
    ...
    list.wEvent_ListItemActivated do (event: wEvent):
        ...
        let
            i = event.getIndex()
            dialog = list.TextEntryDialog(caption = "✎ Edit",
                    message = list.getItemText(i, 0), value = list.getItemText(
                    i, 1).strip(), style = wModalFrame)
        var v: string
    ...
    save.wEvent_Button do ():
        ...
        frame.MessageDialog("Settings Saved!", "✍️ Save", wOk or
    ...
    search.wEvent_Button do ():
        results = searchSettings(list, search.TextEntryDialog(
                caption = "🔎 Search", message = "",
                style = wModalFrame).display())
        list.setFocus()
    ...
    repo.wEvent_Button do ():
        results = clearSearchResults(list, results)
        ShellExecute(0, "open", "https://github.com/Aetopia/Halo-Infinite-Settings-Editor",
                nil, nil, 0)
        list.setFocus()
    ...
    about.wEvent_Button do ():
        ...
        panel.MessageDialog("Created by Aetopia\nhttps://github.com/Aetopia/Halo-Infinite-Settings-Editor",
                "About", wOk or wIconInformation).display()
        list.setFocus()
